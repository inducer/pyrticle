...
<\body>
  ...
  <\hide-part|3>
    ...
    <math|\<rho\><rprime|'>(\<b-x\><rprime|'>)> in <math|K<rprime|'>> by
    shape function reconstruction. Assume we have \ a shape function <math|S>
    satisfying
    ...
    <with|color|red|An important question arises here:> The scheme used in
    the code right now does not do what Figure
    <reference|fig:dilation-lab-flat> says. Instead, we deposit particle
    shapes as round blobs in the lab frame, and so they appear strangely
    elongated in the rest frame, as shown in Figure <reference|fig:dilation>.
    This does not seem to be the correct thing to do--if the ``electrons''
    have proper rotational symmetry anywhere, then it should be in their rest
    frame. The treatment of Figure <reference|fig:dilation-lab-flat> seems
    more appropriate, where charge blobs shortened by <math|\<gamma\>> are
    deposited in the lab frame, resulting in the ``correct'' shape in the
    rest frame. However, option <reference|fig:dilation-lab-flat> obviously
    makes charge deposition in the lab frame harder, because it necessitates
    more mesh resolution in the <math|z> direction by a factor of
    <math|\<gamma\>>. Can this be helped somehow? Can we get away with option
    <reference|fig:dilation> and not worry about it? Am I correct in
    asserting that Picture <reference|fig:dilation-lab-flat> is more
    physically correct?

    <\big-figure>
      \ <postscript|pic-dilation-2.fig|11cm|||||>
    </big-figure|<label|fig:dilation>Present charge deposition scheme.
    Produces elongated particles in the rest frame.>

    \;

    Note that we never want to explicitly deal with a function defined on a
    mesh in the rest frame, since we do not have (and do not want to
    construct) such a mesh. Instead, we treat functions living in the rest
    frame on the (existing) lab frame mesh, and simply adapt any rest frame
    derivative operators to act on lab frame functions as if the function was
    ``properly'' in the rest frame. To this end, we define
    ...
    <math|\<b-x\>>-dependencies switchted to the <math|K<rprime|'>> frame,
    but still carry the values they do in <math|K>, i.e. no Lorentz transform
    has taken place.
    ...
    If the charge distribution <math|\<rho\>> only depends on the radial
    coordinate <math|r>, we may compute <math|\<b-E\>> outside the charge
    cloud exactly by using Gauÿ' law. We know <math|div
...
<\references>
  <\collection>
    ...
    <associate|fig:dilation|<tuple|2|?|#3>>
    <associate|fig:dilation-lab-flat|<tuple|1|?|#3>>
  </collection>
</references>
